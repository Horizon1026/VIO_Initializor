...
bool Backend::EstimateGyroBiasByMethodOneForInitialization() {
    ...
    for (uint32_t iter = 0; iter < max_iteration; ++iter) {
        ...
        auto new_frame_iter = std::next(data_manager_->imu_based_frames().begin());
        for (uint32_t i = min_frames_idx; i < max_frames_idx; ++i) {
            // Localize the frame with bias in 'imu_based_frames_' between frame i and i + 1.
        ...
        for (auto &frame: data_manager_->imu_based_frames()) {
    ...
    // Report result.
    const Vec3 bias_g = data_manager_->imu_based_frames().back().imu_preint_block.bias_gyro();
...
bool Backend::EstimateGyroBiasByMethodTwoForInitialization() {
    ReportInfo("[Backend] Try to estimate bias of gyro by Method 2, which is the raw version from HeYijia, but is not "
               "valid now.");
